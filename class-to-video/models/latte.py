...
class Latte_skip(nn.Module):
    ...
    def __init__(
        ...
        self.skip_norms = nn.ModuleList(
            [
                FP32_Layernorm(2 * hidden_size, elementwise_affine=True, eps=1e-6) for d in range(depth//4)
            ]
        )
        
        self.skip_linears = nn.ModuleList(
            [
                ResidualLinear(hidden_size) for d in range(depth//4)
            ]
        )
    ...
    def forward(self, 
        ...
        for i in range(0, len(self.blocks), 2):
            ...
            if i >= len(self.blocks)//2:
                skip_linear = self.skip_linears[(i-14)//2]
                skip_norm = self.skip_norms[(i-14)//2]
                skip = skips.pop()
    ...
    def forward_with_save(self, 
        ...
        import copy
        if len(self.feature_log) == 0:
            self.feature_log.append(copy.deepcopy(x))
        else:
            last_val = self.feature_log.pop()
            delta = F.cosine_similarity(x.float(), last_val.float(), dim=0).mean().item()
            # torch.log(torch.norm(x.float()-last_val.float(), p=2)).item()
            self.feature_log.append(delta)
            self.feature_log.append(copy.deepcopy(x))
    ...
    def forward_with_cfg_with_save(self, x, t, y=None, cfg_scale=7.0, use_fp16=False, text_embedding=None):
        """
        Forward pass of Latte, but also batches the unconditional forward pass for classifier-free guidance.
        """
        # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb
        ...
        if use_fp16:
            combined = combined.to(dtype=torch.float16)
        model_out = self.forward_with_save(combined, t, y=y, use_fp16=use_fp16, text_embedding=text_embedding)
        # For exact reproducibility reasons, we apply classifier-free guidance on only
        # three channels by default. The standard approach to cfg applies it to all channels.
        # This can be done by uncommenting the following line and commenting-out the line following that.
        # eps, rest = model_out[:, :self.in_channels], model_out[:, self.in_channels:]
...
class Latte_skip_cache(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    def __init__(
        ...
        self.cache_cont = 0
        self.skip_N = skip_N
        self.skip_at_branch = skip_at_branch
    ...
    def forward(self, 
        ...
        if self.extras == 2:
            ...
            text_embedding_temp = repeat(text_embedding, 'n d -> (n c) d', c=self.pos_embed.shape[1])
        skips = []
        skip_at_branch = self.skip_at_branch # [1,2,3,...,28]
        skip_N = self.skip_N
        close_deepcache_at = 50
        start_deepcache_at = 900
        if not self.force_cache and (self.cache_cont % skip_N == 0 or self.cache_val is None or cur_time >= start_deepcache_at):
            for i in range(0, len(self.blocks), 2):
                spatial_block, temp_block = self.blocks[i:i+2]
                if self.extras == 2:
                    c = timestep_spatial + y_spatial
                    ...
                    c = timestep_spatial
                if i == len(self.blocks) - skip_at_branch * 2:
                    self.cache_val = x
                if i >= len(self.blocks)//2:
                    skip_linear = self.skip_linears[(i-14)//2]
                    skip_norm = self.skip_norms[(i-14)//2]
            ...
            if self.force_cache:
                self.cache_val = torch.zeros_like(x)

            for i in range(0, len(self.blocks), 2):
                if not (i < skip_at_branch*2 or i >= len(self.blocks)-skip_at_branch*2):
                    continue
                spatial_block, temp_block = self.blocks[i:i+2]
                if self.extras == 2:
                    c = timestep_spatial + y_spatial
        ...
        if cur_time < close_deepcache_at:
...
def Latte_XL_2_skip(**kwargs):
    return Latte_skip(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

def Latte_XL_2_skip_cache(**kwargs):
    return Latte_skip_cache(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

def Latte_XL_2_skip_cache_force(**kwargs):
    return Latte_skip_cache(depth=28, hidden_size=1152, patch_size=2, num_heads=16, force_cache=True, **kwargs)


def Latte_XL_2_skip_cache_2(**kwargs):
    return Latte_skip_cache(depth=28, hidden_size=1152, patch_size=2, num_heads=16, skip_N=2, skip_at_branch=1, **kwargs)

def Latte_XL_2_skip_cache_3(**kwargs):
    return Latte_skip_cache(depth=28, hidden_size=1152, patch_size=2, num_heads=16, skip_N=3, skip_at_branch=1, **kwargs)

def Latte_XL_2_skip_cache_4(**kwargs):
    return Latte_skip_cache(depth=28, hidden_size=1152, patch_size=2, num_heads=16, skip_N=4, skip_at_branch=1, **kwargs)

def Latte_XL_2_skip_cache_5(**kwargs):
    return Latte_skip_cache(depth=28, hidden_size=1152, patch_size=2, num_heads=16, skip_N=5, skip_at_branch=1, **kwargs)

def Latte_XL_2_skip_cache_6(**kwargs):
    return Latte_skip_cache(depth=28, hidden_size=1152, patch_size=2, num_heads=16, skip_N=6, skip_at_branch=1, **kwargs)
...
Latte_models = {
    ...
    'Latte-skip': Latte_XL_2_skip, 'Latte-skip-cache': Latte_XL_2_skip_cache, 
    'Latte-skip-cache-2': Latte_XL_2_skip_cache_2, 'Latte-skip-cache-3': Latte_XL_2_skip_cache_3,'Latte-skip-cache-4': Latte_XL_2_skip_cache_4, 'Latte-skip-cache-5': Latte_XL_2_skip_cache_5, 'Latte-skip-cache-6': Latte_XL_2_skip_cache_6
}
...
if __name__ == '__main__':
    ...
    network = Latte_XL_2_skip().to(device)
    from thop import profile
    total_flops = []
    for i in range(50):
        t = 1000-20*(1+i)
        t = torch.tensor([t, t, t]).to(device)
        flops, params = profile(network, inputs= (img, t))
        print('FLOPs = ' + str(flops/1000**3) + 'G')
        total_flops.append(flops)
        # print('Params = ' + str(params/1000**2) + 'M')
    print(f'sum flops {sum(total_flops)/1000**4} T')
