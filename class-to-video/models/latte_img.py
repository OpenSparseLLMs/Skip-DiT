...
class Latte_skip_partial(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    def __init__(
        ...
        self.skip_norms = nn.ModuleList(
            [
                FP32_Layernorm(2 * hidden_size, elementwise_affine=True, eps=1e-6) for d in range(self.skip_depth//2)
            ]
        )
        
        self.skip_linears = nn.ModuleList(
            [
                ResidualLinear(hidden_size) for d in range(self.skip_depth//2)
            ]
        )
        num_patches = self.x_embedder.num_patches
        # Will use fixed sin-cos embedding:
    ...
    def forward(self, x, t, y=None, use_fp16=False, y_image=None, use_image_num=0):
        ...
        for i in range(0, len(self.blocks), 2):
            ...
            if self.extras == 2:
                ...
                c = timestep_spatial
            if i >= len(self.blocks)-self.skip_depth:
                skip_linear = self.skip_linears[(len(self.blocks)-i)//2-1]
                skip_norm = self.skip_norms[(len(self.blocks)-i)//2-1]
                skip = skips.pop()
                cat = torch.cat([x, skip], dim=-1)
                cat = skip_norm(cat)
                x = skip_linear(cat)
...
def Latte_Skip_Partial(**kwargs):
    return Latte_skip_partial(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
...
LatteIMG_models = {
    ...
    'LatteIMG-skip': Latte_XL_2_skip, 'LatteIMG-skip-partial': Latte_Skip_Partial
}
