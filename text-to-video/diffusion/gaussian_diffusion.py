    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                x_t = rearrange(x_t, 'b c f h w -> b f c h w').contiguous()
                model_output = rearrange(model_output, 'b c f h w -> b f c h w').contiguous()
                x_start = rearrange(x_start, 'b c f h w -> b f c h w').contiguous()
                noise = rearrange(noise, 'b c f h w -> b f c h w').contiguous()
                B, F, C = x_t.shape[:3]
