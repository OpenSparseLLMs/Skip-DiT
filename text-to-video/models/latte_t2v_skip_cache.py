...
class LatteT2V_skip_cache(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        video_length: int = 16,
        cache_gap: int = 2,
        cache_at_timesteps: list=[700,50]
    ):
        super().__init__()
        self.cache_gap = cache_gap
        ...
        if self.cache_cont % skip_N == 0 or self.cache_val is None or cur_time >= start_deepcache_at:
            ...
            for i, (spatial_block, temp_block) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks)):
                ...
                if i >= 14:
                    ...
                    cat = skip_norm(cat)
                    hidden_states = skip_linear(cat, hidden_states)
            ...
            for i, (spatial_block, temp_block) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks)):
                # 28 layers in total
                if not (i < skip_at_branch or i >= len(self.transformer_blocks)-skip_at_branch):
                    continue
                if i >= 14:
                    skip_linear = self.skip_linears[i-14]
                    skip_norm = self.skip_norms[i-14]
...
if __name__ == '__main__':
    ...
    network = LatteT2V_skip_cache.from_pretrained("/mnt/petrelfs/share_data/cgj/Latte-1", subfolder="transformer", low_cpu_mem_usage=False, device_map=None, video_length=16).to(device,  dtype=torch.float16)
    ...
    for i in range(50):
        t = 1000-20*(1+i)
        t = torch.tensor([t, t]).to(device,  dtype=torch.float16)
