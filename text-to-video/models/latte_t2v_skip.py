...
class Skip_FeedForward(nn.Module):
    r"""
    A feed-forward layer.
...
class ResidualLinear(nn.Module):
    def __init__(self, hidden_states_size, record=False):
        super(ResidualLinear, self).__init__()
        self.linear = nn.Linear(2*hidden_states_size, hidden_states_size)
        self.record = record
        if self.record:
            self.dic = {'in_val':[], 'cat_val':[], 'out_val':[]}
            self.mean = {}
            self.diff = []
            self.diff_mean = None


    def forward(self, cat, hidden_states, index=0):
        x = self.linear(cat)
        if self.record:
            self.dic['cat_val'].append(torch.norm(x, p=2).item())
            self.dic['in_val'].append(torch.norm(hidden_states, p=2).item())
        # if index < 13:
        ...
        output = x
        if self.record:
            self.dic['out_val'].append(torch.norm(output, p=2).item())
            self.diff.append(torch.norm(output.float()-hidden_states.float(), p=2).item())
            self.mean['cat_val'] = sum(self.dic['cat_val'])/len(self.dic['cat_val'])
            self.mean['in_val'] = sum(self.dic['in_val'])/len(self.dic['in_val'])
            self.mean['out_val'] = sum(self.dic['out_val'])/len(self.dic['out_val'])
            self.diff_mean = sum(self.diff)/len(self.diff)
        return output

class LatteT2V_skip(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.skip_norms = nn.ModuleList(
            [
                FP32_Layernorm(2 * inner_dim, elementwise_affine=True, eps=1e-6) for d in range(num_layers//2)
            ]
        )
        
        self.skip_linears = nn.ModuleList(
            [
                ResidualLinear(inner_dim) for d in range(num_layers//2)
            ]
        )
    ...
    def forward(
        ...
        record = False
        record_path = '/mnt/petrelfs/chenguanjie/cgj/Latte-skip/features/skip-dit-v4/'
        if record:
            os.makedirs(record_path, exist_ok=True)
        for i, (spatial_block, temp_block) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks)):
            # 28 layers in total
            if i >= 14:
                skip_linear = self.skip_linears[i-14]
                skip_norm = self.skip_norms[i-14]
                skip = skips.pop()
                cat = torch.cat([hidden_states, skip], dim=-1)
                cat = skip_norm(cat)
                hidden_states = skip_linear(cat, hidden_states, i-14)
            ...
            if self.training and self.gradient_checkpointing:
                ...
                if record:
                    torch.save(hidden_states, os.path.join(record_path+'spatial'+f'-{cur_time}'+f'-{i}.pt'))
                ...
                if enable_temporal_attentions:
                    ...
                    if use_image_num != 0 and self.training:
                        ...
                        if record:
                            torch.save(hidden_states, os.path.join(record_path+'temporal'+f'-{cur_time}'+f'-{i}.pt'))
...
if __name__ == '__main__':
    ...
    network = LatteT2V_skip.from_pretrained("/mnt/petrelfs/share_data/cgj/Latte-1", subfolder="transformer", low_cpu_mem_usage=False, device_map=None, video_length=16).to(device,  dtype=torch.float16)
    h = torch.randn(2, 4, 16, 64, 64).to(device, dtype=torch.float16)
    t = torch.tensor([980, 980]).to(device,  dtype=torch.float16)
    e =  torch.randn(2, 32, 4096).to(device,  dtype=torch.float16)

    from thop import profile 
    for i in range(5):
        flops, params = profile(network, inputs= (h,t, e))
