...
def main(args):
    ...
    import pandas as pd
    dimension_dir = args.prompt_path
    for dim_txt in os.listdir(dimension_dir):
        dim = dim_txt.replace('.txt', '')
        dim_dir = os.path.join(args.save_img_path, dim)
        if not os.path.exists(dim_dir):
            os.makedirs(dim_dir, exist_ok=True)
            
        with open(os.path.join(dimension_dir, dim_txt), 'r') as f:
            prompt_list = f.readlines()
        all_prompts = prompt_list
        prompt_index = 1
        for _, prompt in enumerate(all_prompts):
            prompt = prompt.rstrip('\n')
            if prompt_index % dist.get_world_size() == rank:
                print('{}: Processing the ({}) prompt'.format(prompt_index, prompt))
                for _index in range(5):
                    torch.manual_seed(args.seed+_index)
                    save_path = os.path.join(args.save_img_path, dim, f'{prompt.strip()}-{_index}.mp4')
...
# '/mnt/petrelfs/chenguanjie/cgj/VBench/prompts/prompts_per_dimension'