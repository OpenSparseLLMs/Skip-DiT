...
class LattePipeline(DiffusionPipeline):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, latents=None, debug=False):
    ...
    def __call__(
        ...
        if run_infer_steps is None:
            run_infer_steps = num_inference_steps
        with self.progress_bar(total=run_infer_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # noise_pred = self.transformer(latent_model_input,encoder_hidden_states=prompt_embeds,timestep=current_timestep,added_cond_kwargs=added_cond_kwargs, enable_temporal_attentions=enable_temporal_attentions,return_dict=False)[0]
                # perform guidance
                if do_classifier_free_guidance:
