...
**All the codes and checkpoints will be publicly available at [huggingface](https://huggingface.co/GuanjieChen/Skip-DiT/tree/main) and [github](https://github.com/OpenSparseLLMs/Skip-DiT.git)**

## Skip-DiT and Skip-Cache
![pipeline](visuals/pipeline.jpg)
Illustration of **Skip-DiT** and **Skip-Cache**. At the $t$ step, $x'_{l-1}$ is generated by reusing the features cached at the $t-1$ step, and the DiT blocks 1 to $L-1$ are not executed for more efficient inference.
