...
class HunYuanDiT(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def forward(self,
        ...
        # tensor([981., 981.], device='cuda:0', dtype=torch.float16)
        ...
        record = False
        if record:
            record_path = '/mnt/petrelfs/chenguanjie/cgj/Latte-skip/features/hunyuan/'
        ...
        for layer, block in enumerate(self.blocks):
            ...
            if record:
                torch.save(x, os.path.join(record_path, f'{cur_time}'+f'-{layer}.pt'))
...
if __name__ == '__main__':
    ...
    network = HunYuanDiT(args, depth=40, hidden_size=1408, patch_size=2, num_heads=16, mlp_ratio=4.3637, **kwargs).to(device,  dtype=torch.float16)
    x = torch.randn(2, 4, 128, 128).to(device, dtype=torch.float16)
    e1 =  torch.randn(2, 77, 1024).to(device,  dtype=torch.float16)
    m1 = torch.ones(2,77).to(device,  dtype=torch.float16)
    e1 =  torch.randn(2, 256, 2048).to(device,  dtype=torch.float16)
    m1 = torch.ones(2,256).to(device,  dtype=torch.float16)
    c1 = torch.randn(4096, 88).to(device,  dtype=torch.float16)
    c2 = torch.randn(4096, 88).to(device,  dtype=torch.float16)
    from thop import profile 
    total_flops = []
    for i in range(50):
        t = 1000-20*(1+i)
        t = torch.tensor([t, t]).to(device,  dtype=torch.float16)
        flops, params = profile(network, inputs= (x, t, e1, m1, e2, m2, None, None, c1, c2))
